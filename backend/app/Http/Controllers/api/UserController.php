...
class UserController extends Controller
{
    ...
    {
        $user = Auth::user()->id;
        $request = Store::where('user_id', $user)->where('status', 'approved')->orWhere('status', 'opening')->orWhere('status', 'closing')->select('status')->first();
        if ($request) {
    ...
    public function getPaidShareBill(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $bills = DB::table('share_bills')
                ->select('share_bills.id', 'transactions.created_at', 'share_bills.transaction_id', 'share_bills.amount')
